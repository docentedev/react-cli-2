    ...
    if (!fs.existsSync(path.join(currentExecDir, 'store'))) {
        fs.promises.mkdir(path.join(currentExecDir, 'store'))
            .then(() => {
                ...
                if (!fs.existsSync(path.join(currentExecDir, 'store', 'modules'))) {
                    fs.promises.mkdir(path.join(currentExecDir, 'store', 'modules'))
                        .then(() => {
                            const storeConstName = path.join(currentExecDir, 'store', 'modules', 'index.js');
